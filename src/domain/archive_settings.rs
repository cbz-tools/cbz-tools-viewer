...
mod tests {
    ...
    #[test]
    fn load_book_state_skips_invalid_books_and_keys() {
        ...
        if let Some(dir) = path.parent() {
            std::fs::create_dir_all(dir).expect("create book_state dir");
        }
        std::fs::write(
            &path,
            r#"{
                ...
                "books": {
                    "C:\\books\\alpha.cbz": {
                        ...
                        "quality_override": "balanced",
                        "reading_direction_override": "left_to_right"
                    },
                    "C:\\books\\broken.cbz": {
                        "cover_blank": true,
                        "spread_mode": "unknown",
                    ...
                    "\u0000bad": {
                        "cover_blank": false,
                        "spread_mode": "single",
                        "slideshow_interval_secs": 1.0,
                        "quality_override": null,
                        "reading_direction_override": null
                    }
                }
            }"#,
        )
        .expect("write book_state.json");

        let store = SettingsStore::load();
        let good = store.get(Path::new(r"C:\books\alpha.cbz"));
        assert!(good.cover_blank);
        assert_eq!(good.spread_mode, SpreadMode::Spread);
        assert_eq!(good.slideshow_interval_secs, 5.0);
        assert_eq!(good.quality_override, Some(ViewerQuality::Balanced));
        assert_eq!(
            good.reading_direction_override,
            Some(ReadingDirection::LeftToRight)
        );

        let broken = store.get(Path::new(r"C:\books\broken.cbz"));
        assert!(!broken.cover_blank);
        assert_eq!(broken.spread_mode, SpreadMode::Auto);
        assert_eq!(
            broken.slideshow_interval_secs,
            DEFAULT_SLIDESHOW_INTERVAL_SECS
        );
        assert_eq!(broken.quality_override, None);
        assert_eq!(broken.reading_direction_override, None);
    }

    #[test]
    fn load_book_state_empty_when_books_is_not_object() {
        let _guard = SAVE_TEST_LOCK.lock().unwrap();
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let _local_app_data = LocalAppDataGuard::set(temp_dir.path());
        ...
        std::fs::write(&path, r#"{"schema_version":1,"books":[{"bad":"shape"}]}"#)
            .expect("write book_state.json");

        let store = SettingsStore::load();
        let loaded = store.get(Path::new(r"C:\books\alpha.cbz"));
        assert!(!loaded.cover_blank);
        assert_eq!(loaded.spread_mode, SpreadMode::Auto);
        assert_eq!(
            loaded.slideshow_interval_secs,
            DEFAULT_SLIDESHOW_INTERVAL_SECS
        );
    }

    #[test]
    fn load_book_state_rejects_unknown_schema_version() {
        let _guard = SAVE_TEST_LOCK.lock().unwrap();
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let _local_app_data = LocalAppDataGuard::set(temp_dir.path());
        ...
        std::fs::write(&path, r#"{"schema_version":2,"books":{}}"#).expect("write book_state.json");
    ...
    #[test]
    fn load_book_state_empty_on_broken_root_json() {
        let _guard = SAVE_TEST_LOCK.lock().unwrap();
        let temp_dir = tempfile::tempdir().expect("create temp dir");
        let _local_app_data = LocalAppDataGuard::set(temp_dir.path());
        ...
        std::fs::write(&path, r#"{"schema_version":1,"books":{"oops""#)
            .expect("write broken book_state.json");
