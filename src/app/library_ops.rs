...
mod tests {
    ...
    fn archive_entry(path: PathBuf) -> LibraryEntry {
        LibraryEntry::Archive(crate::domain::archive::BookMeta {
            id: BookId::from_path(path.as_path()),
            path: Arc::<std::path::Path>::from(path.into_boxed_path()),
            title: Arc::<str>::from("archive"),
            size: 1,
    ...
    fn folder_book_entry(path: PathBuf) -> LibraryEntry {
        LibraryEntry::FolderBook(crate::domain::archive::FolderMeta {
            path: Arc::<std::path::Path>::from(path.into_boxed_path()),
            title: Arc::<str>::from("folder-book"),
            modified: SystemTime::UNIX_EPOCH,
        })
    }

    fn folder_entry(path: PathBuf) -> LibraryEntry {
        LibraryEntry::Folder(crate::domain::archive::FolderMeta {
            path: Arc::<std::path::Path>::from(path.into_boxed_path()),
            title: Arc::<str>::from("folder"),
            modified: SystemTime::UNIX_EPOCH,
        })
    }

    fn image_entry(path: PathBuf) -> LibraryEntry {
        LibraryEntry::ImageFile(crate::domain::archive::ImageFileMeta {
            path: Arc::<std::path::Path>::from(path.into_boxed_path()),
            title: Arc::<str>::from("image"),
            size: 1,
            modified: SystemTime::UNIX_EPOCH,
        })
    }
    ...
    #[test]
    fn navigation_book_helpers_include_archive_and_folder_book_only() {
        let archive_path = PathBuf::from(r"C:\books\a.cbz");
        let folder_book_path = PathBuf::from(r"C:\books\b");
        let folder_path = PathBuf::from(r"C:\books\dir");
        let image_path = PathBuf::from(r"C:\books\page.jpg");

        let archive = archive_entry(archive_path.clone());
        let folder_book = folder_book_entry(folder_book_path.clone());
        let folder = folder_entry(folder_path.clone());
        let image = image_entry(image_path.clone());

        assert_eq!(
            App::library_navigation_book_path(&archive),
            Some(archive_path.clone())
        );
        assert_eq!(
            App::library_navigation_book_path(&folder_book),
            Some(folder_book_path.clone())
        );
        assert_eq!(App::library_navigation_book_path(&folder), None);
        assert_eq!(App::library_navigation_book_path(&image), None);

        assert_eq!(
            App::library_navigation_book_id(&archive),
            Some(BookId::from_path(archive_path.as_path()))
        );
        assert_eq!(
            App::library_navigation_book_id(&folder_book),
            Some(BookId::from_path(folder_book_path.as_path()))
        );
        assert_eq!(App::library_navigation_book_id(&folder), None);
        assert_eq!(App::library_navigation_book_id(&image), None);
    }
}
