...
struct InteractiveCacheApplyContext<'a> {
    nav_id: u64,
    physical_page: u32,
    page_left: Option<u32>,
    page_right: Option<u32>,
...
impl ViewerState {
    ...
    fn compute_interactive_request_pages(
        ...
        let mut push_candidate = |candidate_page: u32| {
            ...
            let layout =
                self.view_layout_for_with_caller(candidate_page, display_w, display_h, false);
            let render_signature = self.render_signature_for_decode(
                layout.page_decode_w,
                layout.page_decode_h,
                max_tex_side,
            );
            ...
            let bg_admission_state = self
                .request
                .bg_admission_state
                .get(&key)
                .copied()
                .unwrap_or(BgAdmissionState::Unknown);
    ...
    #[allow(dead_code)]
    pub(super) fn build_interactive_request_plan(
        &mut self,
        display_w: u32,
        display_h: u32,
        max_tex_side: u32,
    ) -> WorkingSetPlan {
        let plan = self.cached_interactive_request_pages(display_w, display_h, max_tex_side);
        if !plan.pages().is_empty() {
            let first = plan
                .pages()
                .first()
                .copied()
                .expect("non-empty pages() was checked just above");
            tracing::trace!(
                "[interactive_request] anchor_page={} direction={} page_count={} admissible_count={} first_page={} render_signature.quality={:?} render_signature.target_w={} render_signature.target_h={} render_signature.max_tex_side={}",
                plan.anchor_page.navigation_page(),
                plan.direction.as_str(),
                plan.page_count(),
                plan.admissible_count(),
                first.page,
                first.render_signature.quality,
                first.render_signature.target_w,
                first.render_signature.target_h,
                first.render_signature.max_tex_side
            );
        } else {
            tracing::trace!(
                "[interactive_request] anchor_page={} direction={} page_count=0 admissible_count=0",
                plan.anchor_page.navigation_page(),
                plan.direction.as_str()
            );
        }
    ...
    #[allow(dead_code)]
    fn try_apply_rgba_cache_hit(&mut self, request: InteractiveCacheApplyContext<'_>) -> bool {
        let InteractiveCacheApplyContext {
            nav_id,
            physical_page,
            page_left,
            page_right,
        ...
        let view_idx = physical_page;
        let mut left_hit = page_left.is_none();
        let mut right_hit = page_right.is_none();
        let mut left_reuse_reason: Option<&'static str> = None;
        let mut right_reuse_reason: Option<&'static str> = None;
        let mut left_cached_size: Option<(u32, u32)> = None;
        let mut right_cached_size: Option<(u32, u32)> = None;
        let bg_rgba_cache = self.request.worker_manager.bg_rgba_cache();
        // 長押し相当の連続入力中は、キャッシュヒットでも即コミットしない。
        // loading=true を維持してプレースホルダを優先表示する。
        if self.ui_runtime.nav_consecutive_count >= 2 {
            tracing::trace!(
                "[viewer-cache-apply-skip] reason=consecutive_gate nav_id={} view={} page_left={:?} page_right={:?} target_w={} target_h={} max_tex_side={} nav_consecutive_count={} left_hit={} right_hit={}",
                nav_id,
                view_idx,
                page_left,
                page_right,
        ...
        let left_frames = match page_left {
            Some(page) => {
                let key = self.rgba_cache_key(page, target_w, target_h, max_tex_side);
                let left_exact_hit = bg_rgba_cache
                    .read()
                    .ok()
                    .is_some_and(|cache| cache.contains(&key));
                let left_tolerance_match = bg_rgba_cache
                    .read()
                    .ok()
                    .and_then(|cache| cache.has_tolerance_match(&key));
                let left_miss_reason = if left_exact_hit {
                    "exact"
                } else if left_tolerance_match.is_some() {
                    "tolerance"
                } else if bg_rgba_cache
                    .read()
                    .ok()
                    .is_some_and(|cache| cache.has_page_variant(&key))
                {
                    "size-mismatch"
                } else {
                    "no-entry"
                ...
                tracing::trace!(
                    "[viewer-rgba-key-lookup] reason=try_apply page={} request_display_w=na request_display_h=na lookup_decode_w={} lookup_decode_h={} key_decode_w={} key_decode_h={} quality={:?} max_tex_side={} exact_hit={} tolerance_hit={} matched_w={:?} matched_h={:?} cache_miss_reason={}",
                    page,
                    target_w,
                    target_h,
                    target_w,
                    ...
                    max_tex_side,
                    left_exact_hit,
                    left_tolerance_match.is_some(),
                    left_tolerance_match.map(|m| m.0),
                    left_tolerance_match.map(|m| m.1),
                    left_miss_reason
                );
                match bg_rgba_cache
                    .write()
                    .ok()
                    .and_then(|mut cache| cache.get_with_tolerance(&key))
                {
                    Some((frames, reuse_reason, (cached_w, cached_h))) => {
                        left_hit = true;
                        left_reuse_reason = Some(reuse_reason);
                        left_cached_size = Some((cached_w, cached_h));
                        Some(frames)
                    }
                    None => {
                        tracing::trace!(
                            "[viewer-cache-apply-skip] reason=left_miss nav_id={} view={} page_left={:?} page_right={:?} target_w={} target_h={} max_tex_side={} nav_consecutive_count={} left_hit={} right_hit={}",
                            nav_id,
                            view_idx,
                            page_left,
                            page_right,
        ...
        let right_frames = match page_right {
            Some(page) => {
                let key = self.rgba_cache_key(page, target_w, target_h, max_tex_side);
                let right_exact_hit = bg_rgba_cache
                    .read()
                    .ok()
                    .is_some_and(|cache| cache.contains(&key));
                let right_tolerance_match = bg_rgba_cache
                    .read()
                    .ok()
                    .and_then(|cache| cache.has_tolerance_match(&key));
                let right_miss_reason = if right_exact_hit {
                    "exact"
                } else if right_tolerance_match.is_some() {
                    "tolerance"
                } else if bg_rgba_cache
                    .read()
                    .ok()
                ...
                tracing::trace!(
                    ...
                    max_tex_side,
                    right_exact_hit,
                    right_tolerance_match.is_some(),
                    right_tolerance_match.map(|m| m.0),
                    right_tolerance_match.map(|m| m.1),
                    right_miss_reason
                );
                match bg_rgba_cache
                    .write()
                    ...
                    .and_then(|mut cache| cache.get_with_tolerance(&key))
                {
                    Some((frames, reuse_reason, (cached_w, cached_h))) => {
                        right_hit = true;
                        right_reuse_reason = Some(reuse_reason);
                        right_cached_size = Some((cached_w, cached_h));
                        Some(frames)
                    }
                    None => {
                        tracing::trace!(
                            "[viewer-cache-apply-skip] reason=right_miss nav_id={} view={} page_left={:?} page_right={:?} target_w={} target_h={} max_tex_side={} nav_consecutive_count={} left_hit={} right_hit={}",
                            nav_id,
                            view_idx,
                            page_left,
                            page_right,
        ...
        tracing::trace!(
            "[viewer-cache-apply-hit] reason=full_hit nav_id={} view={} page_left={:?} page_right={:?} target_w={} target_h={} max_tex_side={} nav_consecutive_count={} left_hit={} right_hit={} left_reuse_reason={:?} right_reuse_reason={:?} left_cached_size={:?} right_cached_size={:?} requested_size=({},{})",
            nav_id,
            view_idx,
            page_left,
            page_right,
        ...
        self.display_assets.content_left =
            left_frames.map(|frames| PageContent::from_frames(frames, "viewer_left", ctx));
        self.register_gpu_texture_history(
            page_left,
            Self::static_texture_payload(self.display_assets.content_left.as_ref()),
            render_signature,
        );
        self.display_assets.content_right =
            right_frames.map(|frames| PageContent::from_frames(frames, "viewer_right", ctx));
        self.register_gpu_texture_history(
            page_right,
            Self::static_texture_payload(self.display_assets.content_right.as_ref()),
            render_signature,
        ...
        self.persistent.displayed_page = view_idx;
        self.record_reading_session_display_commit(page_left, page_right);
        ...
        let req = self.request.loader.peek_next_request_id();
        tracing::trace!(
            "[viewer-upload] nav_id={} req={} view={} upload_ms={}",
            nav_id,
            req,
            view_idx,
            0
        ...
        let (total_nav_ms, request_left_hit, request_right_hit) = self
            ...
            .unwrap_or(("none".to_owned(), "none", "none"));
        tracing::trace!(
            "[viewer-nav-commit] nav_id={} req={} view={} total_nav_ms={} request_left_hit={} request_right_hit={} final_left_ready={} final_right_ready={} decode_ms=0 upload_ms=0 queue_wait_ms=0",
            nav_id,
            req,
            view_idx,
            total_nav_ms,
            request_left_hit,
            request_right_hit,
            self.display_assets.content_left.is_some(),
            self.display_assets.content_right.is_some()
        );
        tracing::trace!(
            view_idx,
            page_left = ?page_left,
            ...
            target_w,
            target_h,
            "viewer_ui: rgba cache hit applied"
        );
        true
    }
