...
mod tests {
    ...
    #[test]
    fn split_args_keeps_quoted_path() {
        let args = "--json \"C:\\Program Files\\Tool\\file.cbz\" --flag";
        let parsed = split_args_safely(args).expect("parse");
        assert_eq!(
            parsed,
            vec![
                "--json".to_owned(),
                "C:\\Program Files\\Tool\\file.cbz".to_owned(),
                "--flag".to_owned()
            ]
        );
    }
    ...
    #[test]
    fn split_args_supports_japanese_path() {
        let args = "--input \"C:\\path\\to\\sample.cbz\"";
        let parsed = split_args_safely(args).expect("parse");
        assert_eq!(
            parsed,
            vec!["--input".to_owned(), "C:\\path\\to\\sample.cbz".to_owned()]
        );
    }

    #[test]
    fn build_args_expands_paths_placeholder() {
        let paths = vec![
            PathBuf::from(r"C:\a\1.cbz"),
            PathBuf::from(r"C:\a\2.cbz"),
            PathBuf::from(r"C:\a\3.cbz"),
        ];
        let args = build_args_from_template("--batch {paths}", &paths).expect("build args");
        assert_eq!(
            args,
            vec![
                "--batch".to_owned(),
                r"C:\a\1.cbz".to_owned(),
                r"C:\a\2.cbz".to_owned(),
                r"C:\a\3.cbz".to_owned(),
            ]
        );
    }

    #[test]
    fn build_args_expands_path_placeholder_for_multi_select() {
        let paths = vec![
            PathBuf::from(r"C:\a\1.cbz"),
            PathBuf::from(r"C:\a\2.cbz"),
            PathBuf::from(r"C:\a\3.cbz"),
        ...
        let args =
            build_args_from_template("--mode optimize \"{path}\"", &paths).expect("build args");
        assert_eq!(
            args,
            vec![
                "--mode".to_owned(),
    ...
    #[test]
    fn build_args_expands_path_placeholder_for_single_select() {
        let paths = vec![PathBuf::from(r"C:\a\single.cbz")];
        let args = build_args_from_template("\"{path}\"", &paths).expect("build args");
        assert_eq!(args, vec![r"C:\a\single.cbz".to_owned()]);
    }
}
