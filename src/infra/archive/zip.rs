...
mod tests {
    ...
    /// 最小限の stored ZIP を手動で組み立てて ZipReader をテスト
    fn make_stored_zip(filename: &str, content: &[u8]) -> Vec<u8> {
        let mut buf = Vec::new();

        // ローカルファイルヘッダ（offset 0 から開始）
        let lh_off = buf.len() as u32; // = 0
        let fname = filename.as_bytes();
        buf.extend_from_slice(&SIG_LOCAL.to_le_bytes());
        buf.extend_from_slice(&20u16.to_le_bytes()); // version needed
        buf.extend_from_slice(&(1u16 << 11).to_le_bytes()); // flags: UTF-8
        buf.extend_from_slice(&0u16.to_le_bytes()); // stored
        buf.extend_from_slice(&0u16.to_le_bytes()); // mod time
        buf.extend_from_slice(&0u16.to_le_bytes()); // mod date
        buf.extend_from_slice(&0u32.to_le_bytes()); // crc32 (dummy)
        buf.extend_from_slice(&(content.len() as u32).to_le_bytes()); // comp size
        buf.extend_from_slice(&(content.len() as u32).to_le_bytes()); // unc size
        buf.extend_from_slice(&(fname.len() as u16).to_le_bytes());
        buf.extend_from_slice(&0u16.to_le_bytes()); // extra len
        buf.extend_from_slice(fname);
        // ここからが圧縮データ（local_data_offset で計算される位置）
        ...
        buf.extend_from_slice(&lh_off.to_le_bytes()); // local header offset（0）
        buf.extend_from_slice(fname);
    ...
    #[test]
    fn test_stored_zip_round_trip() {
        let content = b"\xFF\xD8\xFF\xE0fake jpeg data";
        let zip_data = make_stored_zip("001.jpg", content);

        // tempfile に書いて ZipReader::open
        let mut tmp = tempfile::NamedTempFile::new().unwrap();
        tmp.write_all(&zip_data).unwrap();

        let reader = ZipReader::open(tmp.path()).unwrap();
        assert_eq!(reader.page_count(), 1);
        let first_page = reader.read_page_n(0).unwrap();
        assert_eq!(&first_page[..], content);

        let bytes = reader.read_first_image().unwrap();
        assert_eq!(&bytes[..], content);
    }

    #[test]
    fn test_dir_entry_excluded() {
        let content = b"data";
        let zip_data = make_stored_zip("subdir/", content);
        let mut tmp = tempfile::NamedTempFile::new().unwrap();
        tmp.write_all(&zip_data).unwrap();

        let reader = ZipReader::open(tmp.path()).unwrap();
        assert_eq!(reader.page_count(), 0, "directory entries must be excluded");
    }

    #[test]
    fn test_non_image_excluded() {
        let zip_data = make_stored_zip("readme.txt", b"hello");
        let mut tmp = tempfile::NamedTempFile::new().unwrap();
        tmp.write_all(&zip_data).unwrap();
    ...
    #[test]
    fn test_bmp_and_gif_are_included_by_extension_filter() {
        let bmp_zip = make_stored_zip("001.bmp", b"BMdummy");
        let gif_zip = make_stored_zip("002.gif", b"GIF89adummy");

        let mut bmp_tmp = tempfile::NamedTempFile::new().unwrap();
        bmp_tmp.write_all(&bmp_zip).unwrap();
        let bmp_reader = ZipReader::open(bmp_tmp.path()).unwrap();
        assert_eq!(bmp_reader.page_count(), 1);
        assert_eq!(&bmp_reader.read_first_image().unwrap()[..], b"BMdummy");

        let mut gif_tmp = tempfile::NamedTempFile::new().unwrap();
        gif_tmp.write_all(&gif_zip).unwrap();
        let gif_reader = ZipReader::open(gif_tmp.path()).unwrap();
        assert_eq!(gif_reader.page_count(), 1);
        assert_eq!(&gif_reader.read_first_image().unwrap()[..], b"GIF89adummy");
    }
}
