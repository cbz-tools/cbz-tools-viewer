...
mod tests {
    ...
    fn temp_nav_dir() -> PathBuf {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("system time before epoch")
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("cbz-nav-test-{unique}"));
    ...
    #[test]
    fn move_target_path_from_insertion_index_uses_gap_anchor() {
        let paths = vec![PathBuf::from("A.cbz"), PathBuf::from("C.cbz")];

        assert_eq!(
            move_target_path_from_insertion_index(&paths, 1, NavDirection::Next, false),
            Some(PathBuf::from("C.cbz"))
        );
        assert_eq!(
            move_target_path_from_insertion_index(&paths, 1, NavDirection::Previous, false),
            Some(PathBuf::from("A.cbz"))
        );
        assert_eq!(
            move_target_path_from_insertion_index(&paths, 0, NavDirection::Previous, false),
            None
        );
    }

    #[test]
    fn list_supported_navigation_books_in_dir_includes_archives_and_folder_books_only() {
        let dir = temp_nav_dir();
        let archive = dir.join("B.cbz");
        let folder_book = dir.join("A-book");
        let folder = dir.join("C-folder");
        let image = dir.join("D.jpg");

        fs::create_dir_all(&folder_book).expect("create folder-book dir");
        fs::create_dir_all(&folder).expect("create folder dir");
        write_file(&folder_book.join("page1.jpg"));
        write_file(&archive);
        write_file(&image);
        write_file(&folder.join("note.txt"));

        let books = list_supported_navigation_books_in_dir(&archive);
        assert_eq!(books, vec![folder_book.clone(), archive.clone()]);

        let books_from_dir = list_supported_navigation_books_in_dir(&folder_book);
        assert_eq!(books_from_dir, vec![folder_book.clone(), archive.clone()]);
