...
* アーカイブ内のアニメーション WebP と GIF をそのままストリーミング再生できます。見開き表示にも対応し、通常画像と同じようにページ移動できます。APNG の再生にも対応します。
...
Library と Viewer はそれぞれ単独で動作でき、IPC を通じて連携します。

```text
Library                              Viewer
┌────────────────────┐              ┌────────────────────┐
│ 閲覧 / 検索         │              │ 閲覧               │
│ コレクション管理     │◀──── IPC ───▶│ 前の本 / 次の本     │
│ 本の並び順           │              │ 検索 / 削除         │
│ 読書状態             │              │ アーカイブ再構築     │
└────────────────────┘              └────────────────────┘
        単独動作                            単独動作
...
連携時、Viewer は Library の現在の本の並び順に従い、コレクション関連の操作や読書状態を共有します。

この分離構成はマルチモニターにも適しており、一方のディスプレイに Library、もう一方に Viewer を表示して利用できます。
